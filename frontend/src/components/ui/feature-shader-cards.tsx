...
const features: FeatureCardData[] = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-6 h-6 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M12 21a9 9 0 100-18 9 9 0 000 18ptM9 10a2 2 0 11-4 0 2 2 0 014 0zM19 10a2 2 0 11-4 0 2 2 0 014 0zM14 17a2 2 0 11-4 0 2 2 0 014 0z" />
      </svg>
    ),
    shader: {
      proportion: 0.4,
...
export default function FeatureShaderCards() {
  ...
  return (
    <section className="relative w-full py-20 lg:py-28 px-4 sm:px-6 lg:px-8 bg-neutral-950/90 text-white overflow-hidden border-t border-white/5">
      {/* Background Radial Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-blue-600/10 rounded-full blur-[140px] pointer-events-none" aria-hidden="true" />

      <div className="max-w-7xl mx-auto relative z-10">
        
        {/* Section Header */}
        <div className="text-center mb-16 lg:mb-20 space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-400/20 text-cyan-400 text-[11px] font-bold uppercase tracking-[0.2em] shadow-inner">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
            MULESCOPE INTELLIGENCE
          </div>

          <h2 className="text-3xl md:text-4xl lg:text-5xl font-black uppercase tracking-tight text-white drop-shadow-md">
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-6 lg:gap-8 max-w-5xl mx-auto">
...
function FeatureCard({ feature, reduceMotion }: { feature: FeatureCardData; reduceMotion: boolean }) {
  ...
  return (
    <div className="relative group h-full min-h-[340px] flex flex-col rounded-3xl overflow-hidden border border-white/10 bg-neutral-950/80 backdrop-blur-xl hover:border-cyan-400/50 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_rgba(56,189,248,0.2)]">
      
      {/* Background Animated WebGL Shader */}
      {!reduceMotion && !hasWebGLError && (
        <div className="absolute inset-0 opacity-40 group-hover:opacity-60 transition-opacity duration-700 pointer-events-none rounded-3xl overflow-hidden">
      ...
      {/* Glass Overlay Shader Fallback Pattern */}
      <div className="absolute inset-0 bg-gradient-to-b from-neutral-900/50 via-neutral-950/80 to-neutral-950/95 pointer-events-none" />
      ...
      <div className="relative z-10 p-6 sm:p-8 flex flex-col h-full justify-between flex-1">
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-5">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-400/90 bg-cyan-500/10 px-2.5 py-1 rounded-full border border-cyan-400/20">
              {feature.eyebrow}
            </span>
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-600/10 border border-cyan-400/30 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:border-cyan-400/60 transition-all duration-300">
              {feature.icon}
            </div>
          </div>

          {/* Feature Title */}
          <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-white mb-3 group-hover:text-cyan-200 transition-colors">
            {feature.title}
          </h3>

          {/* Feature Description */}
          <p className="text-xs sm:text-sm text-blue-100/70 leading-relaxed mb-6 font-normal">
            {feature.description}
          </p>
        </div>
        ...
        <div>
          <div className="flex flex-wrap gap-1.5 mb-6">
            {feature.chips.map((chip, idx) => (
              <span
                key={idx}
                className="text-[10px] font-bold tracking-wider px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-cyan-200/80 group-hover:border-cyan-400/30 group-hover:text-cyan-300 transition-colors"
              >
                {chip}
              </span>
            ))}
          ...
          {/* Learn More Link */}
          <div className="flex items-center text-xs font-bold text-cyan-400 group-hover:text-cyan-300 transition-colors">
            <span className="mr-2 uppercase tracking-wider text-[10px]">Capability Specs</span>
