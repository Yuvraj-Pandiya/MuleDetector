...
export default function FeatureShaderCards() {
  ...
  return (
    <section className="relative w-full py-16 lg:py-24 px-4 sm:px-6 lg:px-8 bg-black text-white overflow-hidden border-t border-white/10 font-sans antialiased flex flex-col items-center justify-center">
      ...
      <div className="w-full max-w-[1450px] mx-auto relative z-10 flex flex-col items-center text-center">
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-14 lg:mb-18 space-y-4 w-full">
          ...
          <p className="text-sm md:text-base lg:text-lg text-zinc-400 max-w-3xl mx-auto font-normal leading-relaxed text-center">
...
function FeatureCard({ feature, reduceMotion }: { feature: FeatureCardData; reduceMotion: boolean }) {
  ...
  return (
    <div className="relative group w-full h-full min-h-[350px] flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-gradient-to-b from-zinc-900/80 via-zinc-950/90 to-black backdrop-blur-xl hover:border-white/30 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_12px_35px_rgba(255,255,255,0.06)]">
      ...
      <div className="relative z-10 p-6 lg:p-7 flex flex-col justify-between flex-1 h-full">
        ...
        <div className="flex flex-col text-left">
          ...
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-zinc-300 bg-white/5 px-3 py-1 rounded-full border border-white/10">
              {feature.eyebrow}
            </span>
            <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/15 flex items-center justify-center shadow-md group-hover:scale-110 group-hover:border-white/40 transition-all duration-300 shrink-0">
              {feature.icon}
            </div>
          </div>

          {/* Feature Title */}
          <h3 className="text-xl lg:text-2xl font-bold tracking-tight text-white mb-2.5 group-hover:text-zinc-200 transition-colors">
            {feature.title}
          </h3>

          {/* Feature Description */}
          <p className="text-xs lg:text-sm text-zinc-400 font-normal leading-relaxed mb-6">
            {feature.description}
          </p>
        </div>
        ...
        <div className="mt-auto pt-2 text-left">
          <div className="flex flex-wrap gap-1.5 mb-5">
            {feature.chips.map((chip, idx) => (
              <span
                key={idx}
                className="text-[10px] font-semibold tracking-wide px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-zinc-300 group-hover:border-white/20 group-hover:text-white transition-colors"
              >
                {chip}
              </span>
            ))}
